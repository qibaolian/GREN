...
class chest_Xray(imdb):
    def __init__(self, image_set, fold, cls_ratio, box_ratio):
        ...
        self._data_path = '/data2/qbl/chest_x_ray'
    ...
    def image_path_from_index(self, index):
        ...
        return image_path
    # read hash_image file 
    def hash_image_path_from_index(self, index):
        """
        Construct an image path from the image's "index" identifier.
        """
        image_path = os.path.join(self._data_path, 'image_block', index)
        assert os.path.exists(image_path), 'Path does not exist: {}'.format(image_path)
        return image_path
    ...
    def gt_roidb(self):
        ...
        if self._cls_ratio != '0.0':
            cls_anno_file = osp.join(self._data_path, 'cls_annotations_' + self._cls_ratio + '.json')
